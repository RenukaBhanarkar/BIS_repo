...
class Miscellaneouswinnerwall extends CI_Controller
{
    ...
    public function index()
    {
     ...
     public function winner_wall_dashbaord()
    { 
        $this->load->view('admin/headers/admin_header');
        $this->load->view('winnerwall/winner_wall_dashbaord' );
        $this->load->view('admin/footers/admin_footer');
    }



    public function miscellaneous_winner_wall_list()
    {  
        $data = array();
        $data['winnerwall'] = $this->Miscellaneous_winnerwall_model->getWinnerWallList();
        ...
        $this->load->view('admin/headers/admin_header');
        $this->load->view('winnerwall/miscellaneous_winner_wall_list',$data);
        $this->load->view('admin/footers/admin_footer');
    }
 
    public function miscellaneous_winner_wall_form()
    {  
     
        $this->load->view('admin/headers/admin_header');
        $data['prises']=$this->Miscellaneous_winnerwall_model->prises();
        $data['competation']=$this->Miscellaneous_winnerwall_model->published_quiz();

        if ($this->form_validation->run('miscellaneous_winner_wall_form') == FALSE) 
        {
            $this->load->view('winnerwall/miscellaneous_winner_wall_form',$data);
        } 
        else 
        { 
            ...
            $insertMasterData = $this->Miscellaneous_winnerwall_model->checkWinnerMaster($quiz_id);
            if (empty($insertMasterData)) 
            {
                ...
                $this->Miscellaneous_winnerwall_model->insertWinnerMaster($formdata2);
            }
            $dataadd = $this->Miscellaneous_winnerwall_model->insertWinnerwall($formdata);
            if ($dataadd) {
                $data1['status'] = 1;
...
public function displayWall()
    {
        ...
        $details = array();
        // $details = '';
        $details = $this->Miscellaneous_winnerwall_model->displayWall($submit_id);
        if (empty($details)) {
            $data['status'] = 0;
    ...
    public function getWinnerData()
    {
        ...
        $details = array(); 
        $details = $this->Miscellaneous_winnerwall_model->getWinnerData($id);
        if (empty($details)) {
            $data['status'] = 0;
            $data['message'] = 'Failed to get details , Please try again.';
            $data['data'] = $details;
    ...
    public function viewWallWinner()
    {
        ...
        $details = array();
        // $details = '';
        $details = $this->Miscellaneous_winnerwall_model->viewWallWinner($quiz_id);
        if (empty($details)) {
            $data['status'] = 0;
            $data['message'] = 'Failed to get details , Please try again.';
            $data['data'] = $details;
    ...
    public function DeleteData(){
        try {   
            ...
            $id = $this->input->post('id');
            $id = $this->Miscellaneous_winnerwall_model->DeleteData($id);
            if ($id) {
                $data['status'] = 1;
        ...
        redirect(base_url() . "winnerwall/miscellaneous_winner_wall_form", 'refresh');
    }
     ...
     public function submitWinnerWall(){
        try {   
            ...
            $submit_id = $this->input->post('submit_id');
            $id = $this->Miscellaneous_winnerwall_model->submitWinnerWall($id);
            if ($id) {
                $data['status'] = 1;
                $data['message'] = 'Deleted successfully.';
     ...
     public function updateWinnerWall(){
        try {
            ...
            $id = $this->Miscellaneous_winnerwall_model->updateWinnerWall($formdata,$id);
            if ($id) {
                $data['status'] = 1;
        ...
        redirect(base_url() . "winnerwall/miscellaneous_winner_wall_list", 'refresh');
    }
    ...
    public function deleteWinnerWall(){
        try {   
            ...
            $id = $this->input->post('id');
            $quiz_id = $this->input->post('quiz_id');
            $delId = $this->Miscellaneous_winnerwall_model->deleteWinnerWall($id,$quiz_id);
            if ($delId) {
                $data['status'] = 1;
                $data['message'] = 'Deleted successfully.';
    ...
    public function miscellaneous_winner_wall_archive()
    {
        $data = array();
        $data['winnerwall'] = $this->Miscellaneous_winnerwall_model->getArchiveWinnerWallList();
 
        $this->load->view('admin/headers/admin_header');
        $this->load->view('winnerwall/miscellaneous_winner_wall_archive',$data);
        $this->load->view('admin/footers/admin_footer');
    }

    public function miscellaneous_winner_wall_view($id)
    {
        $id= encryptids("D", $id);
        $data = array();
        $data['masterWinners'] = $this->Miscellaneous_winnerwall_model->getMasterWinnerWall($id);
 
        $this->load->view('admin/headers/admin_header');
        $this->load->view('winnerwall/miscellaneous_winner_wall_view',$data);
        $this->load->view('admin/footers/admin_footer');
    }

    public function miscellaneous_winner_wall_edit($id)
    {
        $this->load->view('admin/headers/admin_header');
        $id= encryptids("D", $id);
        $data['masterWinners']= $this->Miscellaneous_winnerwall_model->getMasterWinnerWall($id);
        ...
        if ($this->form_validation->run('miscellaneous_winner_wall_form') == FALSE) 
        {
            $this->load->view('winnerwall/miscellaneous_winner_wall_edit',$data);
        } 
        else 
        {  
            if (!file_exists('uploads/winnerwall')) { mkdir('uploads/winnerwall', 0777, true); }
            ...
            $updatedata = $this->Miscellaneous_winnerwall_model->updateWinnerwallDetails($formdata,$wall_id);
            if ($updatedata) {
                $data1['status'] = 1;
                $data1['message'] = 'Deleted successfully.'; 
    ...
    public function miscellaneous_winner_wall_edit2()
    {   
